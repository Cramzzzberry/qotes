define(['exports'], function (t) {
    ...
    findMatchingRoute({ url: t, sameOrigin: e, request: s, event: n }) {
      ...
      for (const r of i) {
        ...
        if (o)
          return (
            (i = o),
            ((Array.isArray(i) && 0 === i.length) || (o.constructor === Object && 0 === Object.keys(o).length) || 'boolean' == typeof o) &&
              (i = void 0),
            { route: r, params: i }
          )
      }
  ...
  function h(t, e, n) {
    ...
    if ('string' == typeof t) {
      ...
      if (!(t instanceof i)) throw new s('unsupported-route-type', { moduleName: 'workbox-routing', funcName: 'registerRoute', paramName: 'capture' })
      o = t
    }
  ...
  class b {
    ...
    async fetch(t) {
      ...
      try {
        ...
        throw (i && (await this.runCallbacks('fetchDidFail', { error: t, event: e, originalRequest: i.clone(), request: r.clone() })), t)
      }
    }
    ...
    async getCacheKey(t, e) {
      ...
      if (!this.u[s]) {
        let n = t
        for (const t of this.iterateCallbacks('cacheKeyWillBeUsed')) n = L(await t({ mode: e, request: n, event: this.event, params: this.params }))
        this.u[s] = n
      }
      return this.u[s]
    }
  ...
  class C {
    ...
    async U(t, e, n) {
      ...
      try {
        if (((i = await this._(e, t)), !i || 'error' === i.type)) throw new s('no-response', { url: e.url })
      } catch (s) {
    ...
    async L(t, e, s, n) {
      ...
      if ((await e.runCallbacks('handlerDidComplete', { event: n, request: s, response: i, error: r }), e.destroy(), r)) throw r
    }
  }
  class E extends C {
    constructor(t = {}) {
    ...
    k() {
      ...
      for (const [s, n] of this.plugins.entries())
        n !== E.copyRedirectedCacheableResponsesPlugin && (n === E.defaultPrecacheCacheabilityPlugin && (t = s), n.cacheWillUpdate && e++)
      0 === e ? this.plugins.push(E.defaultPrecacheCacheabilityPlugin) : e > 1 && null !== t && this.plugins.splice(t, 1)
    }
  }
  ;(E.defaultPrecacheCacheabilityPlugin = { cacheWillUpdate: async ({ response: t }) => (!t || t.status >= 400 ? null : t) }),
    (E.copyRedirectedCacheableResponsesPlugin = { cacheWillUpdate: async ({ response: t }) => (t.redirected ? await m(t) : t) })
  class O {
    constructor({ cacheName: t, plugins: e = [], fallbackToNetwork: s = !0 } = {}) {
      ;(this.K = new Map()),
        (this.P = new Map()),
        (this.T = new Map()),
        (this.l = new E({ cacheName: f(t), plugins: [...e, new g({ precacheController: this })], fallbackToNetwork: s })),
    ...
    precache(t) {
      this.addToCacheList(t),
        this.W || (self.addEventListener('install', this.install), self.addEventListener('activate', this.activate), (this.W = !0))
    }
    addToCacheList(t) {
      const e = []
      for (const n of t) {
        ...
        if (this.K.has(i) && this.K.get(i) !== t) throw new s('add-to-cache-list-conflicting-entries', { firstEntry: this.K.get(i), secondEntry: t })
